...
/**
 * ダッシュボード本体。認証が有効なときは signedIn（サインイン中のアカウント）が渡され、
 * 権限はそれを使う（ロール切替セレクタは出さない）。未設定のときは従来どおり切替で確認できる。
 */
export function Dashboard({ signedIn }: { signedIn?: CurrentAccount | null }) {
  ...
  // 認証有効時はサインイン中のアカウント、未設定時はロールに対応する代表アカウント。
  const account = signedIn ?? ACCOUNTS[role];
